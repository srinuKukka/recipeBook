...
public class RecipeService implements RecipeRepository {
        ...
        public Recipe updateRecipe(int recipeId, Recipe recipe) {
                Recipe exiestingRecipe = recipeBook.get(recipeId);
                if (exiestingRecipe == null) {
                        throw new ResponseStatusException(HttpStatus.NOT_FOUND);
                }
                if (recipe.getRecipeName() != null) {
                        exiestingRecipe.setRecipeName(recipe.getRecipeName());
                }
                if (recipe.getRecipeType() != null) {
                        exiestingRecipe.SetRecipeType(recipe.getRecipeType());
                }
                if (recipe.getIngredient() != null) {
                        exiestingRecipe.setIngredient(recipe.getIngredient());
                }

                return exiestingRecipe;
        }
